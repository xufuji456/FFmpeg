...
static int inner_remove_dns_cache(char *hostname, DnsCacheEntry *dns_cache_entry) {
    if (context && dns_cache_entry) {
        if (dns_cache_entry->ref_count == 0) {
            ...
            free_private_addrinfo(&dns_cache_entry->addr_info);
...
static DnsCacheEntry *new_dns_cache_entry(char *hostname, struct addrinfo *cur_ai, int64_t timeout) {
    ...
    new_entry->addr_info = (struct addrinfo *) av_mallocz(sizeof(struct addrinfo));
    if (!new_entry->addr_info) {
        av_freep(&new_entry);
        goto fail;
    }

    memcpy(new_entry->addr_info, cur_ai, sizeof(struct addrinfo));

    new_entry->addr_info->ai_addr = (struct sockaddr *) av_mallocz(sizeof(struct sockaddr));
    if (!new_entry->addr_info->ai_addr) {
        av_freep(&new_entry->addr_info);
        av_freep(&new_entry);
        goto fail;
    }

    memcpy(new_entry->addr_info->ai_addr, cur_ai->ai_addr, sizeof(struct sockaddr));
    new_entry->addr_info->ai_canonname = NULL;
    new_entry->addr_info->ai_next      = NULL;
